...
async function onRequest(props) {
    ...
    let response = await indexerApi.get(getPath(props));

    if(response.data.total_count === 0){
        showWithoutMetadata.value = true;
        response = await indexerApi.get(getPath(props));
    }
