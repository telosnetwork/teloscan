    ...
    async created() {
        ...
        try {
            ...
            if (typeof this.traces !== 'undefined' && this.traces !== null) {
                ...
                const query = `/transaction/${this.transaction.hash}/internal?limit=1000&sort=ASC&offset=0&includeAbi=1`;
                const response = await this.$indexerApi.get(query);
                if (response && response.data?.results?.length > 0) {
                    dataset = response.data?.results;
                    this.$contractManager.addContractsToCache(response.data?.contracts);
                } else {
                    console.error(
                        `Could not retrieve internal transactions for transaction ${this.transaction.hash}`,
                        response,
                    );
                    return;
                }
            ...
            for (let k = 0; k < dataset.length; k++) {
                ...
                const contract = await this.getContract(itx.action.to);
                let inputs; let outputs; let args; let name; let
                    isTransferETH = false;
