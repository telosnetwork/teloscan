            ...
            div( v-if="isContract && params.length > 0" class="fit row wrap justify-start items-start content-start")
              ...
              div(class="col")
                div(v-for="param in params" class="fit row wrap justify-start items-start content-start")
                  ...
                  div(v-if="param.arrayChildren" class="col-8 text-wrap")
                    div(v-for="(value, index) in param.value")
                      ...
                      div(v-else  ) {{ value }},
                  div(v-else-if="param.type === 'address'" class="col-8 text-wrap") <AddressField :address="param.value" copy :name="param.value === contract.address && contract.name ?  contract.name : null"   />
                  div(v-else  class="col-8 text-wrap") {{ param.value }}
