        ...
        $route: {
            ...
            async handler(newRoute, oldRoute = {}) {
                if (newRoute !== oldRoute) {
                    ...
                    const newHashIsInvalid = (!tabHashes.includes(newHash) && !subtabHashes.includes(newHash))
    ...
    methods: {
        async loadAccount() {
            ...
            if (contract?.creationInfo?.transaction || contract?.supportedInterfaces?.length > 0) {
                ...
                try {
                    const ethAccount = await this.$evm.telos.getEthAccount(this.accountAddress);
                    this.telosAccount = ethAccount?.account;
                    this.nonce = ethAccount?.nonce;
                } catch (e) {
                    console.info(e);
                }
            }
        ...
        getBalanceDisplay(balance) {
            const strBalance = formatWei(balance, WEI_PRECISION, 4);
        ...
        cancelHide(evt) {
            ...
            [, this.tab] = this.subtab.split('_');
            evt.preventDefault();
            evt.stopPropagation();
            this.$refs.panels.goTo(this.subtab);
        },
        toggleMenus(menu) {
            Object.keys(this.menus).forEach((i) => {
                if (i in this.menus) {
                    this.menus[i] = false;
